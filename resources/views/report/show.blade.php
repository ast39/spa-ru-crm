    ...
    <div class="card bg-primary text-white">
        ...
        <div class="card-body bg-light">
            <table class="table table-striped table-borderless">
                ...
                <tr><td colspan="2" class="bg-light text-center text-secondary"><b>Смена</b></td></tr>
                ...
                <tr><td colspan="2" class="bg-light text-center text-success"><b>Доходы</b></td></tr>

                <tr>
                    <th class="text-start">{{ __('Выручка за программы') }}</th>
                    <td class="text-end">{{ number_format($report->programs_profit, 0, '.', ' ') }} р.</td>
                </tr>
                <tr>
                    <th class="text-start">{{ __('Выручка за услуги') }}</th>
                    <td class="text-end">{{ number_format($report->services_profit, 0, '.', ' ') }} р.</td>
                </tr>
                <tr>
                    <th class="text-start">{{ __('Выручка за напитки') }}</th>
                    <td class="text-end">{{ number_format($report->bar_profit, 0, '.', ' ') }} р.</td>
                </tr>
                <tr><td colspan="2" class="bg-light text-center text-success"></td></tr>
                <tr>
                    <th class="text-start">{{ __('Выручка наличкой') }}</th>
                    <td class="text-end">{{ number_format($report->cash_profit, 0, '.', ' ') }} р.</td>
                </tr>
                <tr>
                    <th class="text-start">{{ __('Выручка безналом') }}</th>
                    <td class="text-end">{{ number_format($report->card_profit, 0, '.', ' ') }} р.</td>
                </tr>
                <tr>
                    <th class="text-start">{{ __('Выручка переводами') }}</th>
                    <td class="text-end">{{ number_format($report->phone_profit, 0, '.', ' ') }} р.</td>
                </tr>
                <tr><td colspan="2" class="bg-light text-center text-success"></td></tr>
                <tr>
                ...
                <tr><td colspan="2" class="bg-light text-center text-danger"><b>Расходы</b></td></tr>

                <tr>
                    <th class="text-start">{{ __('Администратор:') }} {{ $report->shift->openedAdmin->name }}</th>
                    <td class="text-end">-{{ number_format($report->admin_profit, 0, '.', ' ') }} р.</td>
                </tr>

                @forelse($report->shift->masters_profits as $master_id => $data)
                    <tr>
                        <th class="text-start">{{ __('Мастер:') }} {{ $data['name'] }}</th>
                        <td class="text-end">-{{ number_format($data['profit'], 0, '.', ' ') }} р.</td>
                    </tr>
                @empty
                @endforelse
                ...
                <tr>
                    ...
                    <td class="text-end">-{{ number_format($report->expenses, 0, '.', ' ') }} р.</td>
                </tr>
                ...
                <tr>
                    ...
                    <td class="text-end">{{ number_format(0 - $report->expenses - $report->admin_profit - $report->masters_profit - $report->sale_sum, 0, '.', ' ') }} р.</td>
                </tr>

                <tr><td colspan="2" class="bg-light text-center text-primary"><b>Баланс</b></td></tr>
                        ...
                        <div class="accordion-body">
                            <table class="table table-bordered">
                                ...
                                @forelse($report->shift->programs as $program)
                                    ...
                                    <tr>
                                        <td>Оплата {{ Helper::payType($program->pay_type) }} {{ number_format($program->total_price_with_sale, 0, '.', ' ') }} р., программа {{ Helper::seanceStatus($program->status) }}, , {{ $program->sale_sum > 0 ? 'с учетом скидки ' . number_format($program->sale_sum, 0, '.', ' ') . 'р.' : 'без скидки' }}</td>
                                    </tr>
                            ...
                            <table class="table table-bordered">
                                ...
                                @forelse($report->shift->services as $service)
                                    ...
                                    <tr>
                                        ...
                                        @else
                                            <td>Оплата {{ Helper::payType($service->pay_type) }} {{ number_format($service->total_price_with_sale, 0, '.', ' ') }} р, {{ $service->sale_sum > 0 ? 'с учетом скидки ' . number_format($service->sale_sum, 0, '.', ' ') . 'р.' : 'без скидки' }}</td>
                                        @endif
                                    </tr>
                                    <tr><td colspan="2" class="bg-light"></td></tr>
                                @empty
                                ...
                                @forelse($report->shift->bar as $item)
                                    ...
                                    <tr>
                                        @if($item->gift > 0)
                                            <td>{{ __('За счет заведения')  }}</td>
                                        @else
                                            <td>Оплата {{ Helper::payType($item->pay_type) }} {{ number_format($item->total_price_with_sale, 0, '.', ' ') }} р., {{ $item->sale_sum > 0 ? 'с учетом скидки ' . number_format($item->sale_sum, 0, '.', ' ') . 'р.' : 'без скидки' }}</td>
                                        @endif
                                    </tr>
                                    <tr><td colspan="2" class="bg-light"></td></tr>
                                @empty
                                    <div class="text-center p-2 mb-2 bg-secondary bg-gradient text-white rounded">{{ __('Напитки отсутствуют') }}</div>
                                @endforelse
                                </tbody>
                            </table>
                        </div>
