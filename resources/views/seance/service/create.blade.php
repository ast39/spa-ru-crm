            ...
            <form method="post" action="{{ route('shift.service.store') }}">
                ...
                <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="profile-tab-pane" aria-selected="false">Услуга</button>
                    </li>
                    <li class="nav-item" role="presentation">
                ...
                <div class="tab-content" id="myTabContent">
                    ...
                    <div class="tab-pane fade show active" id="profile-tab-pane" role="tabpanel" aria-labelledby="profile-tab" tabindex="0">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="service_id" id="service_id" class="form-select form-control">
                                @foreach($services as $service)
                                    <option data-price="{{ $service->price }}" {{ (request()->service_id ?? 0) == $service->service_id ? 'selected' : '' }} value="{{ $service->service_id }}">{{ $service->title }}</option>
                                @endforeach
                            </select>
                            @error('service_id')
                        ...
                        {{-- Начало услуги --}}
                        <div class="mb-3">
                            <label for="open_time" class="form-label required">{{ __('Начало услуги') }}</label>
                            <div class="mmot-filterline__one mmot-inputwithico-left" data-input_clear_content>
                                ...
                                <input type="datetime-local" class="form-control" id="open_time" name="open_time"
                                       placeholder="{{ __('Время начала') }}" onfocus="(this.showPicker())"
                                       data-input_clear value="{{ old('open_time', now(+2)->format('Y-m-d\TH:i')) }}" >
                            </div>
                            @error('open_time')
                                <p class="text-danger mt-2">{{ $message }}</p>
                            @enderror
                        ...
                        {{-- Окончание услуги --}}
                        <div class="mb-3">
                            <label for="close_time" class="form-label required">{{ __('Окончание услуги') }}</label>
                            <div class="mmot-filterline__one mmot-inputwithico-left" data-input_clear_content>
                                <svg class="mmot-inputwithico-left__ico"><use xlink:href="#site-calendar"></use></svg>
                                <input type="datetime-local" class="form-control" id="close_time" name="close_time"
                                       placeholder="{{ __('Время окончания') }}" onfocus="(this.showPicker())"
                                       data-input_clear value="{{ old('close_time', now(+2)->addHour()->format('Y-m-d\TH:i')) }}" >
                            </div>
                            @error('close_time')
                                <p class="text-danger mt-2">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select  class="form-control form-select" id="status" name="status">
                                <option title="Состоялся" value="1" {{ request()->status == 1 ? 'selected' : null }}>{{ __('Оказана') }}</option>
                    ...
                    <div class="tab-pane fade" id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab" tabindex="0">
                        ...
                        {{-- Основной мастер --}}
                        <div class="mb-3">
                            <label for="master_id" class="form-label required">{{ __('Основной мастер') }}</label>
                        ...
                        <div class="mb-3">
                            <label for="master_percent" class="form-label required">{{ __('% основного мастера') }}</label>
                            <div class="input-group">
                                <button class="btn btn-danger btn-decrement" style="width: 36px" type="button">-</button>
                                <input type="text" class="form-control text-center percent-input" name="master_percent" value="{{ old('master_percent', 40) }}" readonly />
                                <button class="btn btn-success btn-increment" style="width: 36px" type="button">+</button>
                            </div>
                            @error('master_percent')
                                <p class="text-danger mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="cover_master_id" id="cover_master_id" class="form-select form-control">
                                ...
                                @foreach($masters as $master)
                                    <option title="{{ $master->name }}" {{ request()->cover_master_id == $master->id ? 'selected' : '' }} value="{{ $master->id }}">{{ $master->name }}</option>
                                @endforeach
                            </select>
                            @error('cover_master_id')
                                <p class="text-danger mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control text-center percent-input disabled cover_master_percent" disabled id="cover_master_percent" name="cover_master_percent" value="{{ old('cover_master_percent', 40) }}" readonly />
                                <button class="btn btn-success btn-increment disabled cover_master_percent" disabled style="width: 36px" type="button">+</button>
                            </div>
                            @error('cover_master_percent')
                                <p class="text-danger mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="contact-tab-pane" role="tabpanel" aria-labelledby="contact-tab" tabindex="0">
                        ...
                        <div class="mb-3">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control text-end percent-input" id="handle_price" name="handle_price" value="{{ round($services[0]->price) }}" readonly />
                        ...
                        <div class="mb-3">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control text-end percent-input" id="total" name="total" value="{{ old('total', round($services[0]->price)) }}" readonly />
                                <span class="input-group-text">{{ __('руб.') }}</span>
                            </div>
                        </div>
...
@push('js2')
    <script>
        $(document).ready(function() {
            const coverMasterIdCell = $('#cover_master_id');

            togglePercentField();
            coverMasterIdCell.change(function() {
                togglePercentField();
            });
            ...
            function togglePercentField() {
                var selectedMaster = coverMasterIdCell.val();
            ...
            function recalculate() {
                // Базовая цена
                const selectedOption = $('#service_id').find('option:selected');
